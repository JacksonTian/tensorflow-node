...
describe('tensorflow', function () {
  it('version()', function() {
    expect(tf.version()).to.be.ok();
  });

  it('DataType', function () {
    console.log(tf);
    expect(tf.DataType).to.be.ok();
  });

  it('Status', function () {
    var status = new tf.Status();
    var code = status.getCode();
    expect(code).to.be(tf.Code.OK);
    var message = status.getMessage();
    expect(message).to.be('');
    status.set(tf.Code.CANCELLED, 'cancel');
    expect(status.getCode()).to.be(tf.Code.CANCELLED);
    expect(status.getMessage()).to.be('cancel');
    //   TF_DeleteStatus(s);
  });
